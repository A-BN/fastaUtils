...
#' @param contig_df, a contigs data.frame with two cols, name and size
...
nx_lx <- function(contig_df) {
  ...
  L50 <- sum((contig_df$cum_size) < (tot_contig_size * 0.5)) + 1
  N50 <- contig_df$size[L50]
  
  L90 <- sum((contig_df$cum_size) < (tot_contig_size * 0.9)) + 1
  N90 <- contig_df$size[L90]

  metrics_df <- data.frame(x = c(50, 90), n = c(N50, N90), l = c(L50, L90))
  metrics_df$cum_Nx <- contig_df$cum_size[c(L50, L90)]
